...
exports.create = function (req, res, next) {
  ...
  form.uploadDir = process.env.OPENSHIFT_DATA_DIR || __dirname + "/data";
  form.keepExtensions = true;
  form.parse(req, function (err, fields, files) {
    if (!err) {
      console.log('File uploaded : ' + files.avatar.path);
      var newUser = new User(fields);
      newUser.provider = 'local';
      newUser.role = 'user';
      newUser.avatar.data = fs.readFileSync(files.avatar.path);
...
exports.me = function (req, res, next) {
  ...
  User.findOne({
    ...
    if (!!user.avatar && !!user.avatar.contentType) {
      user.avatar.url = user.avatar.data.toString('base64');
    }
    res.json(user);
  });
};
